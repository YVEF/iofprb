...
public:
    ...
    explicit main_canvas(config_state&, const ambient::driveprv&) noexcept;
...
private:
    ...
    std::array<std::unique_ptr<nextback_canvas> (*)(config_state&, const ambient::driveprv&), 2> steps_ = {
            &get_next_canvas_<start_menu>,
            &get_next_canvas_<bench_chart>
    };

    template<typename T>
    static std::unique_ptr<nextback_canvas> get_next_canvas_(config_state& config, const ambient::driveprv& drv)
    {
